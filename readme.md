...
**06.03**
每日一题：837. 新21点（动态规划，dp[i]表示已有i分开始，最终能赢的概率）

**06.04**
每日一题：238. 除自身以外数组的乘积（两次遍历，第一次求i前缀的积，和前缀和差不多，只是不考虑i，第二次求i后缀的积，把两个数组乘起来就行。优化的话可以在一个数组上面做，第一次求前缀积，第二次用R表示后缀积，往前缀积上面乘。）

**06.05**
每日一题：面试题29. 顺时针打印矩阵（定义dx，dy，用for循环或者while循环，退出条件是res的数量到达n*m，循环中判断如果超出边界或者已经访问过，只要转向一次就可以）
